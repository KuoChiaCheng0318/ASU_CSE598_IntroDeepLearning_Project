...
for l in range (0,5):
  ...
  for i in range (17*l,17*l+16):
    #if(int(all_preds[incorrect[i]])==8 and int(y_test[incorrect[i]])==6):
    #ax[k,j].imshow(x_test[incorrect[i]].reshape(224,224), cmap='gray')
    ax[k,j].imshow(x_test[incorrect[i]])
    ax[k,j].set_title("Predicted: " + str(breed_dict2[int(all_preds[incorrect[i]])]) + "\n"+"Actual: " + str(breed_dict2[int(y_test[incorrect[i]])]), fontsize=6)
    ax[k,j].axis('off')
    j+=1
    if(j>=4):
      j=0
